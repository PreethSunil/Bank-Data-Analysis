...
function routePageInitializer() {
    ...
    if (path.includes('/dashboard') || path === '/') {
        ...
        loadModelMetrics();
    } else if (path.includes('/churn-prediction')) {
        // Churn prediction form is ready for user input.
...
function fillChurnSampleProfile() {
    document.getElementById('c-credit-score').value = 600;
    document.getElementById('c-age').value = 52;
    document.getElementById('c-tenure').value = 2;
    document.getElementById('c-balance').value = 125000;
    document.getElementById('c-products').value = 1;
    document.getElementById('c-card').value = 1;
    document.getElementById('c-active').value = 0;
    document.getElementById('c-salary').value = 65000;
    document.getElementById('c-geography').value = 'Germany';
    document.getElementById('c-gender').value = 'Female';
    showToast('Loaded churn-risk sample profile.', 'gold');
}

async function handleChurnPrediction(e) {
    e.preventDefault();
    const button = document.getElementById('churn-predict-submit-btn');
    button.disabled = true;
    button.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Calculating Churn Risk...';
    const payload = {
        credit_score: document.getElementById('c-credit-score').value,
        age: document.getElementById('c-age').value,
        tenure: document.getElementById('c-tenure').value,
        balance: document.getElementById('c-balance').value,
        num_products: document.getElementById('c-products').value,
        has_credit_card: document.getElementById('c-card').value,
        is_active_member: document.getElementById('c-active').value,
        estimated_salary: document.getElementById('c-salary').value,
        geography: document.getElementById('c-geography').value,
        gender: document.getElementById('c-gender').value
    };
    try {
        const response = await fetch('/api/churn-predict', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(payload)});
        const data = await response.json();
        button.disabled = false;
        button.innerHTML = '<i class="fa-solid fa-user-clock"></i> Calculate Churn Risk';
        if (data.error) { showToast(data.error, 'red'); return; }
        document.getElementById('churn-result-placeholder').classList.add('hidden');
        document.getElementById('churn-result-content').classList.remove('hidden');
        const banner = document.getElementById('churn-status-banner');
        banner.className = `status-banner ${data.prediction ? 'rejected' : 'accepted'}`;
        document.getElementById('churn-status-icon').innerHTML = data.prediction ? '<i class="fa-solid fa-triangle-exclamation"></i>' : '<i class="fa-solid fa-circle-check"></i>';
        document.getElementById('churn-status-title').textContent = data.prediction_label;
        document.getElementById('churn-status-subtitle').textContent = `Model classification: ${data.prediction ? 'Class 1 (Churn)' : 'Class 0 (Stay)'}`;
        document.getElementById('churn-probability').textContent = `${data.probability_pct}%`;
        document.getElementById('churn-probability-bar').style.width = `${data.probability_pct}%`;
        document.getElementById('churn-confidence').textContent = `${data.confidence_pct}%`;
        document.getElementById('churn-risk-tier').textContent = data.risk_level;
        document.getElementById('churn-recommendation').textContent = data.recommendation;
        showToast('Churn risk calculated successfully.', 'gold');
    } catch (error) {
        button.disabled = false;
        button.innerHTML = '<i class="fa-solid fa-user-clock"></i> Calculate Churn Risk';
        showToast('Server connection error during churn prediction.', 'red');
    }
}
